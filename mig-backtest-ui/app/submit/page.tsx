      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="glass-card rounded-2xl p-5">
          ...
          <div className="bg-slate-950 rounded-lg p-3 border border-slate-800 text-xs font-mono leading-relaxed overflow-x-auto">
            ...
            <span className="text-slate-300">:</span>
            <br />
            <span className="text-slate-600 ml-4"># prices: (num_stocks, num_days)</span>
            <br />
            <span className="text-slate-600 ml-4"># return: same shape — shares traded</span>
            <br />
            <span className="text-slate-600 ml-4"># +N buy · -N sell/short · 0 hold</span>
            <br />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="glass-card rounded-2xl p-6 flex flex-col justify-center items-center text-center gap-2">
          ...
          {submissionsLoading ? (
            ...
            <p className="text-5xl font-bold text-slate-100 mono-nums">{dailyCount.count}</p>
          )}
          ...
          <div className="flex gap-1.5 mt-2">
            {Array.from({ length: dailyCount.limit }).map((_, i) => (
              <div
                key={i}
                className={`w-6 h-1.5 rounded-full ${submissionsLoading ? "bg-slate-800 animate-pulse" : i < dailyCount.count ? "bg-emerald-500" : "bg-slate-800"}`}
              />
            ))}
          </div>
        </div>
